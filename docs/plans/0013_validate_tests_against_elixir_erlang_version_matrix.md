...
- [x] For each pairing in `matrix.json`, install the versions via `asdf install`, set them active via `asdf set`, run `mix deps.get`, `mix compile`, and `mix test`, and update the entry's `result` field with pass/fail
